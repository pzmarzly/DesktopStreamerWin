...
int TrySend(ZL_ulong IP, ZL_ushort port, ZL_cstring message, ZL_ulong max_ping) {
    ...
    if (select_result == 0) {
        ...
        // safe to send in 1 call as long as SETTINGS_MAX_VAR_LEN (1024) < SO_MAX_MSG_SIZE (8195)
    ...
    if (GetAdaptersInfo(pAdapterInfo, &ulOutBufLen) == NO_ERROR) {
        ...
        while (pAdapter) {
            ...
            while (pAddrStruct) {
                ...
                ZL_ulong netmask = inet_addr(pAddrStruct->IpMask.String); // to test with smaller network - 0xc0ffffff

                // Windows specific - long = int
                // error == int(-1) == uint(uint_max)
                if (   binaryIP != ULONG_MAX
                    && binaryIP != 0
                    && netmask != ULONG_MAX // if mask is 255.255.255.255, we cannot broadcast anyway
                    && netmask != 0) {
                        ZL_ulong subnet = binaryIP & netmask;
                        ZL_ulong subnet_length = SWAP_UINT32(ULONG_MAX & (~netmask));
                        
                        TryBroadcastNetwork(
                            subnet, subnet_length, // TODO: is x.x.x.0 valid IP for a device? if not, then: subnet + 1, subnet_length - 1
